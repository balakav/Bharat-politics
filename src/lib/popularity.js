...
export async function recomputeGovernmentPopularity(scope, stateId = "") {
  const govs = await bharat01.entities.Government.filter({ type: scope === "national" ? "national" : "state", is_active: true });
  ...
  const budgets = await bharat01.entities.Budget.filter({ scope, state_id: stateId });
  ...
  try {
    const projects = await bharat01.entities.DevelopmentProject.list("-created_date", 500);
  ...
  const ministers = await bharat01.entities.Minister.filter({ government_id: gov.id, is_active: true });
  let scandals = 0;
  for (const m of ministers) {
    const invs = await bharat01.entities.Investigation.filter({ target_player_id: m.player_id });
  ...
  // Trend vs previous
  const prev = await bharat01.entities.PopularityScore.filter({ scope: "government", target_id: gov.id });
  ...
  if (prev.length > 0) {
    return await bharat01.entities.PopularityScore.update(prev[0].id, data);
  }
  return await bharat01.entities.PopularityScore.create(data);
}
...
export async function recomputePartyPopularity(partyName, scope = "national", stateId = "") {
  ...
  const now = (await getCurrentGameTime()).toISOString();
  const prev = await bharat01.entities.PopularityScore.filter({ scope: "party", target_id: partyName });
  const prevScore = prev[0]?.score || 50;
  const trend = score > prevScore + 1 ? "up" : score < prevScore - 1 ? "down" : "stable";
  const data = {
  ...
  if (prev.length > 0) return await bharat01.entities.PopularityScore.update(prev[0].id, data);
  return await bharat01.entities.PopularityScore.create(data);
}