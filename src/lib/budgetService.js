...
export async function createBudget(data, actor) {
  ...
  const rec = await base44.entities.Budget.create({ ...data, deficit, status: "draft", ai_score: 0 });
  logAction({
...
export async function runBudgetAI(budgetId) {
  const b = await base44.entities.Budget.get(budgetId);
  ...
  const updated = await base44.entities.Budget.update(budgetId, {
...
export async function presentBudget(budgetId, actor) {
  ...
  await base44.entities.Budget.update(budgetId, { status: "voting" });
  logAction({
